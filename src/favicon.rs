    ...
    fn walk(node: &DomNode, base: &Url, out: &mut Vec<String>) {
        if node.tag_name.as_deref() == Some("link") {
            ...
            if attr("rel").is_some_and(|rel| {
                rel.split_ascii_whitespace().any(|v| {
                    v.eq_ignore_ascii_case("icon") || v.eq_ignore_ascii_case("apple-touch-icon")
                })
            }) {
                if let Some(url) = attr("href").and_then(|href| base.join(href).ok()) {
                    if matches!(url.scheme(), "http" | "https")
                        && out.len() < 8
                        && !out.contains(&url.to_string())
                    {
...
/// ICO can contain PNG or an uncompressed 1/4/8/24/32-bit Windows DIB.
pub fn icon_source(bytes: &[u8]) -> Option<IconSource<'_>> {
    if bytes.len() > MAX_ENCODED_BYTES {
        return None;
    }
    if bytes.get(..4)? != [0, 0, 1, 0] {
        return Some(IconSource::Encoded(bytes));
    }
    ...
    if count == 0 || count > 64 || bytes.len() < 6 + count * 16 {
        return None;
    }
    let mut entries: Vec<_> = (0..count)
        .map(|i| {
            let offset = 6 + i * 16;
            let side = if bytes[offset] == 0 {
                256
            } else {
                bytes[offset] as u32
            };
    ...
    for (_, entry) in entries {
        ...
        let bits = u16::from_le_bytes([image[14], image[15]]) as usize;
        if !matches!(bits, 1 | 4 | 8 | 24 | 32) {
            continue;
        }
        let height = doubled_height / 2;
        let palette_len = if bits <= 8 {
            let declared = word(32) as usize;
            if declared == 0 {
                1usize << bits
            } else {
                declared
        ...
        let Some(palette) = image.get(header..header.saturating_add(palette_len * 4)) else {
            continue;
        };
        let pixel_offset = header + palette.len();
        let stride = (width as usize * bits).div_ceil(32) * 4;
        let size = stride * height as usize;
        let Some(pixels) = image.get(pixel_offset..pixel_offset.saturating_add(size)) else {
            continue;
        };
        let mask_stride = (width as usize).div_ceil(32) * 4;
        let mask =
            image.get(pixel_offset + size..pixel_offset + size + mask_stride * height as usize);
        if bits != 32 && mask.is_none() {
            continue;
        }
        let has_alpha = bits == 32 && pixels.chunks_exact(4).any(|p| p[3] != 0);
        let mut rgba = Vec::with_capacity(width as usize * height as usize * 4);
        let mut invalid = false;
        for y in 0..height as usize {
            let source_y = height as usize - 1 - y;
            let row = &pixels[source_y * stride..][..stride];
            for x in 0..width as usize {
                let p = if bits <= 8 {
                    let bit = x * bits;
                    let index = ((row[bit / 8] >> (8 - bits - bit % 8))
                        & ((1u16 << bits) - 1) as u8) as usize;
                    let Some(color) = palette.get(index * 4..index * 4 + 4) else {
                        invalid = true;
                        break;
                    };
                    ...
                    [
                        ...
                        row[offset + 1],
                        row[offset + 2],
                        if bits == 32 { row[offset + 3] } else { 255 },
                    ]
                };
                let transparent = mask
                    .is_some_and(|m| m[source_y * mask_stride + x / 8] & (0x80 >> (x % 8)) != 0);
...
pub fn tile(width: u32, height: u32, stride: usize, rgba: &[u8]) -> Option<Vec<u8>> {
    ...
    for y in 0..h {
        for x in 0..w {
            ...
            for channel in 0..3 {
                out[dst + channel] = ((rgba[src + channel] as u32 * alpha
                    + out[dst + channel] as u32 * (255 - alpha)
                    + 127)
                    / 255) as u8;
            }
        }
