...
impl Default for Favicon {
    fn default() -> Self { Self { pixels: favicon::fallback(), loading: None } }
}
impl Favicon {
    pub fn navigate(&mut self, request: Option<(String, Vec<String>)>) {
        self.pixels = favicon::fallback();
        self.loading = request.map(|(origin, candidates)| Box::pin(load(origin, candidates)) as Loading);
    }
    pub fn poll(&mut self) -> bool {
        let Some(future) = &mut self.loading else { return false; };
        let Poll::Ready(result) = future.as_mut().poll(&mut Context::from_waker(Waker::noop())) else { return false; };
        self.loading = None;
        match result {
            Ok(pixels) => { self.pixels = pixels; true }
            Err(error) => {
                crate::parser_probe::report_info(format_args!("solara: favicon fallback reason={error}"));
                false
            }
        }
    }
...
async fn load(origin: String, candidates: Vec<String>) -> Result<Vec<u8>, String> {
    ...
    let now = trueos::clock::unix_seconds().unwrap_or(0);
        ...
        if meta.len <= favicon::MAX_RECORD_BYTES as u64 {
            if let Ok(record) = async_fs::read_file(path.as_bytes()).await {
                if let Some(pixels) = favicon::decode_record(&origin, now, &record) {
                    crate::parser_probe::report_info(format_args!("solara: favicon cache-hit origin={origin}"));
                    return Ok(pixels);
                }
            }
        }
    ...
    for url in candidates {
        if url::Url::parse(&url).ok().is_some_and(|u| u.path().ends_with(".svg")) { continue; }
        let result = async {
            let bytes = crate::native_images::fetch_bytes(url.clone()).await?;
            match favicon::icon_source(&bytes).ok_or("unsupported or oversized favicon")? {
                IconSource::Rgba { width, height, pixels } =>
                    favicon::tile(width, height, width as usize * 4, &pixels).ok_or("invalid ICO extent".into()),
                IconSource::Encoded(encoded) => {
                    let format = if encoded.starts_with(b"\x89PNG\r\n\x1a\n") {
                        ...
                        if width > 1024 || height > 1024 { return Err("favicon dimensions exceed limit".into()); }
                        vmedia::ImageFormat::Png
                    } else if encoded.starts_with(b"BM") { vmedia::ImageFormat::Bmp }
                    else { return Err("unsupported favicon encoding".into()); };
                    let decoded = vmedia::decode(format, encoded).await.map_err(|e| format!("icon decode: {e}"))?;
                    favicon::tile(decoded.info.width, decoded.info.height, decoded.info.stride_bytes as usize, &decoded.rgba)
        ...
        match result {
            Ok(pixels) => {
                if let Some(record) = favicon::encode_record(&origin, now, &pixels) {
                    ...
                    crate::parser_probe::report_info(format_args!("solara: favicon fetched origin={origin} cached={}", stored.is_ok()));
                }
                return Ok(pixels);
            }
