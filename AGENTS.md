...
now lives in `assets/fonts/` and supplies deterministic layout measurements.
...
- `cargo run --locked`: parse and lay out all five embedded pages, print timings, and exit without opening a window.
- `cargo run --locked --no-default-features`: run only the original parsing/script proof.
